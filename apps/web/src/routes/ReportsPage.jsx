...
function BookingsReportTab({ from, to }) {
  ...
  const { data: statuses } = useQuery({ queryKey: ["statuses", "booking"], queryFn: () => apiFetch("/statuses?domain=booking") });
  const statusOptions = [{ value: "", label: "All statuses" }, ...(statuses ?? []).map((s) => ({ value: s.code, label: s.label }))];

  const { data, isLoading, isFetching, refetch } = useQuery({
    queryKey: ["reports-bookings", from, to, statusFilter, search, checkoutBasis, page, pageSize],
    queryFn: () =>
      apiFetch(
